...
class DataRecorder():
    def __init__(self, use_led=True, night_led_only=True):
        ...
        self.desired_brightness = 128  # Target brightness level for image capture
        self.brightness_tolerance = 20  # Acceptable range for brightness adjustment
        
        self.desired_exposure_time = 500000  # Desired exposure time in microseconds
        self.exposure_tolerance = 100  # Acceptable range for exposure adjustment
    ...
    def set_exposure_time(self, exposure_time):
        ...
        exposure_tolerance = 100  # Set tolerance for exposure adjustment
        
        self.picam2.set_controls({'ExposureTime': exposure_time})

        for i in range(5):
            actual_exposure = self.picam2.capture_metadata()['ExposureTime']
            print(actual_exposure)
            if abs(actual_exposure - desired_exposure_time) <= exposure_tolerance:
                break
            
    def capture_image_with_quality_check(self):
        """Capture an image and check its quality metrics"""
        try:
            if platform.system() == "Linux" and hasattr(self, 'picam2'):
                ...
                while True:
                    ...
                    if brightness < self.desired_brightness:
                        ...
                        self.set_exposure_time(self.desired_exposure_time)
                    else:
                        self.desired_exposure_time -= 20000  # Decrease exposure time
                        self.set_exposure_time(self.desired_exposure_time)
    ...
    def capture_image(self):
        ...
        try:
            ...
            if platform.system() == "Linux" and hasattr(self, 'picam2'):
                # Capture with Raspberry Pi camera
                self.capture_image_with_quality_check()
                self.picam2.set_controls({"ExposureTime": self.desired_exposure_time})
